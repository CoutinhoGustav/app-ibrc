    ...
    <Card className="mb-3 p-3">
      <View className="flex-row justify-between items-start mb-2">
        <View className="flex-1">
          <Text className="text-base font-bold text-gray-900">{item.turma}</Text>
          <Text className="text-gray-500 text-xs">Prof. {item.professor}</Text>
        </View>
        <View className="bg-blue-100 px-2 py-1 rounded-full">
          <Text className="text-blue-700 text-xs font-bold">{item.presentes}/{item.total}</Text>
        </View>
      </View>
      ...
      <View className="flex-row items-center mt-2 pt-2 border-t border-gray-50">
        ...
        <View className="flex-row items-center flex-1">
          ...
          <Text className="text-gray-500 text-xs ml-1 truncate" numberOfLines={1}>
            {item.visitantes !== '-' ? `Visitantes: ${item.visitantes}` : 'Sem visitantes'}
          </Text>
        </View>
        <Button
          variant="ghost"
          ...
          onPress={() => router.push({ pathname: '/modal' as any, params: { id: item.id } })}
        >
          Editar
        </Button>
      </View>
      ...
      <View className="bg-white px-4 py-3 border-b border-gray-100">
        <View className="flex-row justify-between items-center mb-2">
          <View>
            ...
            <Text className="text-xs text-gray-500 font-medium">Lista de Presenças</Text>
          </View>
          <TouchableOpacity
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="flex-1 items-center justify-center py-20">
            <Text className="text-gray-400 text-sm font-medium">Nenhum registro encontrado</Text>
          </View>
        }
      />
    </SafeAreaView>
